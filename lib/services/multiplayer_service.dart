...
class MultiplayerService with WidgetsBindingObserver {
  ...
  /// 갤러리/카메라로 고른 [file] 을 Supabase Storage `avatars` 버킷에 업로드하고
  /// profiles.avatar_url 을 갱신한다. 반환값은 public URL.
  ///
  /// 경로 규칙: `<user_id>/<millis>.<ext>` — RLS 가 (storage.foldername)[1]
  /// 으로 본인 폴더만 허용. 매번 새 timestamp 라 캐시 무효화도 자연스럽게 됨.
  Future<String> uploadMyAvatar(File file) async {
    ...
    if (_myProfile == null) {
      throw StateError('프로필을 먼저 만들어주세요.');
    }
    // 확장자 — image_picker 가 보통 .jpg / .png 로 떨궈주지만 안전하게 정규화.
    var ext = file.path.split('.').last.toLowerCase();
    if (ext.length > 5 || ext.isEmpty) ext = 'jpg';
    ...
    try {
      ...
      // 이전 사진 정리 — 새 URL 저장 *전* 에 폴더 정리. 실패해도 무시
      // (다음 업로드 / cleanup 작업이 정리).
      final oldUrl = _myProfile?.avatarUrl;
      if (oldUrl != null) {
        unawaited(_deleteAvatarObjectFromUrl(oldUrl));
      }
      ...
      final res = await _sb
          ...
          .single();
      _myProfile = MultiplayerProfile.fromJson(res);
      _peerProfiles[user.id] = _myProfile!;
      _notify();
      return url;
    } on PostgrestException catch (e) {
      throw _mapError(e);
  ...
  /// Storage public URL → bucket path 추출 후 삭제. 실패는 silently swallow.
  Future<void> _deleteAvatarObjectFromUrl(String url) async {
    try {
      // .../storage/v1/object/public/avatars/<path> 형태 → 마지막 `avatars/` 뒤
      ...
      // 삭제 실패해도 무시 — 고아 파일은 나중에 storage cleanup job 으로 처리.
    }
  }
